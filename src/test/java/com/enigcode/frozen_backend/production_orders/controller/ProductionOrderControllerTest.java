...
class ProductionOrderControllerTest {
    ...
    void setup() throws Exception {
        ...
        when(productionOrderService.returnOrder(any(Long.class), any(OrderStatus.class))).thenReturn(responseDTO);
        when(productionOrderService.getProductionOrder(any(Long.class))).thenReturn(responseDTO);
        when(productionOrderService.findAll(any(), any(Pageable.class))).thenReturn(new PageImpl<>(java.util.List.of()));
    }
    ...
    void testRejectOrder_Success() throws Exception {
        ...
        when(productionOrderService.returnOrder(eq(1L), eq(OrderStatus.RECHAZADO))).thenReturn(rejectedDTO);
    ...
    void testRejectOrder_NotFound_ShouldReturn400() throws Exception {
        when(productionOrderService.returnOrder(eq(999L), eq(OrderStatus.RECHAZADO)))
                .thenThrow(new BadRequestException("No se encontró orden de produccion"));
    ...
    void testRejectOrder_NotPending_ShouldReturn400() throws Exception {
        when(productionOrderService.returnOrder(eq(1L), eq(OrderStatus.RECHAZADO)))
                .thenThrow(new BadRequestException("La orden esta en estado APROBADO"));
