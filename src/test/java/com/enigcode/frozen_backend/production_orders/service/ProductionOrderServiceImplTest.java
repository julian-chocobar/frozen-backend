...
class ProductionOrderServiceImplTest {
    ...
    void approveOrder_success() {
        ...
        // Assert
        assertSame(responseDTO, result);
        assertEquals(OrderStatus.APROBADO, productionOrder.getStatus());
    ...
    void approveOrder_orderNotPending_throws() {
        ...
        ProductionOrder productionOrder = ProductionOrder.builder()
                .id(orderId)
                .product(product)
                .status(OrderStatus.APROBADO) // Already approved
                .quantity(100.0)
                .build();
    ...
    void returnOrder_toRejected_success() {
        ...
        // Act
        ProductionOrderResponseDTO result = service.returnOrder(orderId, OrderStatus.RECHAZADO);

        // Assert
        assertSame(responseDTO, result);
        assertEquals(OrderStatus.RECHAZADO, productionOrder.getStatus());
        verify(movementService).createReserveOrReturn(eq(MovementType.DEVUELTO), anyList());
    }
