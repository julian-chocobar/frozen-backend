...
public class SystemConfigurationController {
    ...
    @Operation(summary = "Devolver parametros de configuración", description = "Devuelve la configuracion activa del sistema, o la crea una default en caso de no existir")
    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<SystemConfigurationResponseDTO> getSystemConfiguration() {
        SystemConfigurationResponseDTO systemConfigurationResponseDTO = systemConfigurationService
                .getSystemConfiguration();

        return new ResponseEntity<>(systemConfigurationResponseDTO, HttpStatus.OK);
    }

    @Operation(summary = "Cambiar parametros de working days", description = "Modifica parametros especificos de los working days")
